define(function (require) {
    ...
    //var WcfProxy = require('app/productManage/interface/wcfProxy');
    //require('app/productManage/interface/numberFormat_interface');
    //require('asyncbox');
    require('date_input');
    require('calendar');
    var FormatNumber = require('goldenstand/format.number');
    var kendo = require('kendo.all.min');
    require('kendoculturecn');
    var ko = require('knockout');
    var stepCode = 'sequence';
    ...
    **/
    ...
    function initKo() {
        ...
        var registerEvent = function (dataSource) {
            ...
            $('.PayLimitation').on('keyup', function (e) {
                var value = $(this).val();
                $(this).val(FormatNumber.convertNumberN(1, value));
            });

            $('.PayLimitation').each(function () {
                var $this = $(this);
                if ($this.val() != '') {
                    $this.val(FormatNumber.convertNumberN(1, $this.val()));
                }
            });
            ...
            $('#sortable_div').height('auto'); //$(top).height() - $('#tabHead').height() - $('#divBody').height() - 455
        }
    ...
    function changeDivHeight() {
        var h = $("#sortable_div").height();
        var dish = $("body").height() - 1 - h;
        var wh = $("#divBody").height();
        $(".payment-container").css({ "paddingBottom": h + "px", "height": dish + "px" });
        $(".k-grid").css("height", dish - 54-wh + "px");
        $(".k-grid-content").css("height", $(".k-grid").height() - 36 + "px");
        
    }
    changeDivHeight();
    $("#scenarioNameList").on("click", ".normal_li", function () {      
        changeDivHeight();
    })
