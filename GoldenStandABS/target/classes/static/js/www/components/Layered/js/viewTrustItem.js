...
var m_TrustItemModule;

function AddItem(obj) {
    if (m_TrustItemModule != null){
        m_TrustItemModule.AddItem(obj);
    }
}

function RemoveItem(obj) {
    if (m_TrustItemModule != null) {
        m_TrustItemModule.RemoveItem(obj);
    }
}

function AddBond(obj) {
    if (m_TrustItemModule != null) {
        m_TrustItemModule.AddBond(obj);
    }
}

function RemoveBond(obj) {
    if (m_TrustItemModule != null) {
        m_TrustItemModule.RemoveBond(obj);
    }
}

function addZXItem(obj) {
    if (m_TrustItemModule != null) {
        m_TrustItemModule.addZXItem(obj);
    }
}

function removeZXItem(obj) {
    if (m_TrustItemModule != null) {
        m_TrustItemModule.removeZXItem(obj);
    }
}
    ...
    var TrustItemModule = (function () {
        ...
        function viwModelBinding() {
            ...
            if (trustId == "0") {
                ...
                //console.log($editTrustCodeBtn.get(0));
                //console.log($editTrustCodeBtn.get(0).style);
                //console.log($editTrustCodeBtn.get(0).style.display);
        ...
        function getCodeDictionaryValue(categoryCode, valueShort) {
            ...
            if (m_viewTrustWizard.OptionSource != null) {
                var items = $.grep(m_viewTrustWizard.OptionSource, function (item) {
        ...
        function getItemValuebyItemCode(obj) {
            ...
            var mm = TrustItemModule.ReturnviewModel();
            var abc=['a','b','c']
            var jsontemp = ko.mapping.toJSON(abc);
    ...
    TrustItem.prototype = {
        ...
        update: function () {
            ...
            $.each(obj.displayFields, function (i, n) {
                ...
                var objtemp = m_viewTrustWizard.api.getTemplate(n.Category, n.SPId, n.SPCode, n.SPRItemCode, n.TBId, n.ItemId, n.ItemCode, n.ItemValue, n.DataType, n.UnitOfMeasure, n.Precise);
                tpl.push(objtemp);
            })
            ...
            var objtemp = m_viewTrustWizard.api.getTemplate(Category, spid, spcode, spritemcode, TBId, itemid, itemcode, datastring.substring(0, datastring.length - 1), DataType, UnitOfMeasure, Precise);
            tpl.push(objtemp);
        ...
        getItemValueByCode: function (code) {
            return TrustItemModule.getItemValueByCode(code);
        },

        getItemValuebyItemCode: function (code) {
            return TrustItemModule.getItemValuebyItemCode(code);
        }
    }

    m_TrustItemModule = TrustItemModule;
    return new TrustItem();
