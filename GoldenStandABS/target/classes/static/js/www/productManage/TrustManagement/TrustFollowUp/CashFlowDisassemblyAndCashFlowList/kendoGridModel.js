define(function (require) {
    ...
    var ActionDisplayName = common.getQueryString('ActionDisplayName');
    var SessionId = common.getQueryString('SessionId');
    //require('gs/www/basicData/main')
            ...
            var dataSource = new kendo.data.DataSource($.extend({
                ...
                schema: {
                    ...
                    data: function (response) {
                        if (jQuery.parseJSON(response).data.length == "0") {
                            GSDialog.HintWindow("当前并无归集日")
                        }
                ...
                dataBound: function () {
                    ...
                    gridbtn.each(function (i, v) {
                        if ($(v).attr('id') == 'btnResults') {
                            ...
                            $(this).click(function () {
                                ScheduleDateId = $($(this).parent().siblings().get(1)).html() || $($(this).parent().siblings()).html();
                                var DimReportingDateId = parent.$("#ReportingDate").val();//把选定的基础资产快照日期传给后面现金流拆分的select框中
                                //再次打开拆分界面
                                if (DimReportingDateId) {
                                    DimReportingDateId = DimReportingDateId.replace(/-/g, '')
                                } else {
                                    DimReportingDateId = "";
                                }
                                var page = location.protocol + '//' + location.host + '/GoldenStandABS/www/components/CashflowSplit/CashflowSplit.html?TrustId={0}&TrustCode={1}&ScheduleDateId={2}&schedulePurpose={3}&DimReportingDateId={4}&xd={5}'.format(TrustId, TrustCode, ScheduleDateId.replace(/-/g, ''), schedulePurpose, DimReportingDateId, xd)
                                openNewIframe(page, TrustId + '_cashflowsplit', '现金拆分: ' + TrustId, parent.parent.viewModel ? parent.parent.viewModel : (window.parent.parent.parent.parent.viewModel ? window.parent.parent.parent.parent.viewModel : window.parent.parent.parent.parent.parent.viewModel));
                            })
                        } else if ($(v).attr('id') == 'NewopenImportTrustAsset') {
                            $(this).click(function () {
                                ScheduleDateId = $($(this).parent().siblings().get(1)).html() || $($(this).parent().siblings()).html();
                                var DimReportingDateId = parent.$("#ReportingDate").val();//把选定的基础资产快照日期传给后面现金流拆分的select框中
                                if (DimReportingDateId) {
                                ...
                                console.log(DimReportingDateId);
                                //打开新版拆分界面
                                var page = location.protocol + '//' + location.host + '/GoldenStandABS/www/components/NewCashflowSplit/NewCashflowSplit.html?TrustId={0}&TrustCode={1}&ScheduleDateId={2}&schedulePurpose={3}&DimReportingDateId={4}&xd={5}'.format(TrustId, TrustCode, ScheduleDateId.replace(/-/g, ''), schedulePurpose, DimReportingDateId, xd)
                                if (ActionDisplayName && SessionId) {
                                    page += "&ActionDisplayName={0}&SessionId={1}".format(decodeURI(escape(ActionDisplayName)), SessionId)
                                }
                                openNewIframe(page, TrustId + '_Newcashflowsplit', '新现金拆分: ' + TrustId, parent.parent.viewModel ? parent.parent.viewModel : (window.parent.parent.parent.parent.viewModel ? window.parent.parent.parent.parent.viewModel : window.parent.parent.parent.parent.parent.viewModel));
                            })
                        }
                    })
