define(function (require) {
    ...
    var poolCutId = comm.getQueryString('poolCutId');
    ...
    function initReportDate() {
        ...
        $.ajax({
            ...
            type: "GET",
            async: false,
            url: svcUrl + 'appDomain=' + poolCutId + 'o&executeParams=' + executeParams,
            dataType: "json",
            contentType: "application/xml;charset=utf-8",
            data: {},
            success: function (response) {
    ...
    function reisterEvent() {
        ...
        $("body").on("click", "#ConfigSave", function () {
            $.anyDialog({
                width: 400,
                height: 200,
                title:'设置配置项',
                html: $("#optionList").clone(true).show(),
                changeallow:false
            })
        })
        $("body").on("click", '#saveOptions', function () {
            var value = $(this).prev().val();
            $("#ConfigCode").val(value)
            createConfigXml();
        })
        $("body").on("click", "#LoadConfig", function () {
            $.anyDialog({
                ...
                height: 200,
                title: '加载配置项',
                html: $("#choiceList").clone(true).show(),
                changeallow: false
            })
        })
        $("body").on("click", '#loadOption', function () {
            var value = $(this).prev().val();
            $("#SplitConfigInfo").val(value)
            LoadSplitConfig();
        })
        ...
        $('#btnSubmit').click(function () {
            ...
            sVariableBuilder.AddVariableItem('PoolDBConnectionStr', string.format("Data Source=MSSQL;Initial Catalog={0};Integrated Security=SSPI;", poolCutId), 'String', 1);
            ...
            if (currentRuleNo == 0) {
                if ($("#IsBaseOnLoanTerm").val() == 1) {
                    ...
                    if (!$("#CalculateHeadInterestByDay").val()) {
                        modes[currentRuleNo].CalculateHeadInterestRule = $("#CalculateHeadInterestByDay").val();
                    }
                }
            ...
            if (currentRuleNo == 1) {
                if ($("#IsBaseOnLoanTerm").val() == 1) {
                    ...
                    if (!$("#CalculateHeadInterestByDay").val()) {
                        ...
                        modes[currentRuleNo].AdjustPrincipalByPlan = $("#TrailPeriodsCalRule").val();
                        modes[currentRuleNo].CalculateHeadInterestRule = $("#CalculateHeadInterestByDay").val();
                    }
                }
            ...
            if (currentRuleNo == 2) {
                if ($("#IsBaseOnLoanTerm").val() == 1) {
                    ...
                    if (!$("#AdjustmentOfprincipal").val()) {
                        $('.splitRuleOptionList>li').eq(currentRuleNo).trigger("click")
                        modes[currentRuleNo].AdjustmentOfprincipal = $("#AdjustmentOfprincipal").val()
                    }
                }
            }
            if (currentRuleNo == 3) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.SaveTemplateOfCashflowSplitRulesResult == 1) {
                        ...
                        GSDialog.HintWindow("配置名称重复", function () {
                            $("#ConfigCode").val("")
                            LoadSplitList()
                        });
                    }
    ...
    function LoadSplitList() {
        ...
        promise().then(function (response) {
            ...
            $("#SplitConfigInfo").find("option").remove();
            if (ConfigJson.length == "0") $("#SplitConfigInfo").append("<option>-</option>")
            $.each(ConfigJson, function (i, v) {
                $("#SplitConfigInfo").append("<option value='" + v.CashflowSplitConfigCode + "'>" + v.CashflowSplitConfigCode + "</option>")
    ...
    function LoadSplitConfig() {
        var ConfigCode = $("#SplitConfigInfo").val();
        var Username = sessionStorage.getItem("gs_UserName");
        ...
        promise().then(function (response) {
            ...
            console.log(DefiningInterval, EliminatePrematureAssets)
