define(function (require) {
    ...
    require('anyDialog');
    //var GsAdminPages = require('gsAdminPages');
    var self = this;

    self.ItemListModel = function () {
        var listCategory = {
            ...
            Originator: [
                ...
                { field: "SequenceNo", title: "顺序号", sortable: true, attrHeader: settable.tableTh, attr: settable.tableTd },
                {
                    ...
                    render: function (data) {
                        var html = '<a style="cursor:pointer" onclick="self.ItemListModel.PagerListModule.Edit(' + data.row.ItemId + ',' + data.row.CategoryId + ');"><i class="icon icon-edit btn btn-link" style="padding: 4px;"></i></a>';
                        html += '&nbsp; <a style="cursor:pointer" onclick="self.ItemListModel.PagerListModule.Del(' + data.row.ItemId + ');"><i class="fa fa-trash-o btn btn-link" style="color: #dd0000;padding: 4px;"></i></a>';
                        return html;
                    }
                }
            ]
        ...
        this.PagerListModule = function () {
            ...
            function edit(id, categoryId) {
                $.anyDialog({
                    ...
                    height: 458, // 弹出框内容高度
                    title: '内容设置',	// 弹出框标题
                    url: '../Items/ItemDetail.html?ItemId=' + id + '&CategoryId=' + categoryId + '&random=' + Math.random(),
                    onClose: function () {
                        //关闭的回调 list 的刷新方法
                        //searchByWhere();
                    },
            ...
            function del(id) {
                GSDialog.HintWindowTF("确定删除？删除后将不可用", function () {
                    var executeParam = {
        ...
        function initData(categoryId) {
            ...
            //    height: 500, // 弹出框内容高度
            //    title: '任务设置',	// 弹出框标题
            //    url: '../Items/ItemDetail.html?CategoryId=' + categoryId + '&random=' + Math.random(),
            //    onClose: function () {
            //        //关闭的回调 list 的刷新方法
        ...
        function searchByWhere() {
            var filterWhere = '';
            $('.list-filters .filter').each(function (i,v) {
                var $this = $(this);
                var value = $this.val();
                if (!value||value.length < 1) { return true; }
                var param = $this.attr('name');
                var ItemCode = $("input[name='ItemCode']").val();
                var ItemAliasValue = $("input[name='ItemAliasValue']").val();
                if (ItemCode == "" && ItemAliasValue == "") {
                    GSDialog.HintWindow("请输入相应的编号或者中文名称");
                }
