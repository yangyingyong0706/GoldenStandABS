		...
		renderOptions: {
		    columns: [{
				...
				template: function(dataItem) {
					var desPath = dataItem.desPath;
					var index = desPath.lastIndexOf('\\');
					desPath = desPath.substring(index + 1, desPath.length)
				    //var downloadUrl = `/PoolCut/Files/RequireField/${desPath}`
					var downloadUrl = '/PoolCut/Files/RequireField/' + desPath;
					var html = '<div><a href="{0}">获取记录下载</a>'.format(downloadUrl);
					html += '</div>';
					return html;
				},
				title: '下载',
