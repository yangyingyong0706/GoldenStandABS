...
define(function (require) {
    ...
    var displayColumns = {
        Originator: [
            {
                field: "assetTypeId", title: "资产类型ID", sortable: true, attrHeader: settable.tableTh, attr: settable.tableTd
            },

            {
                field: "assetType", title: "资产类型", sortable: true, attrHeader: settable.tableTh, attr: settable.tableTd
            },
...

            {
                ...
                render: function (data) {
                    var html = '<a style="cursor:pointer" onclick="self.PagerListModule.Edit(' + data.row.id + ');">编辑</a>';
                    html += '&nbsp; <a style="cursor:pointer" onclick="self.PagerListModule.Del(' + data.row.id + ');">删除</a>';
                    return html;
                }
            }
        ]
    ...
    var PagerListModule = function () {
        ...
        function edit(id) {
            $.anyDialog({
                ...
                height: 500, // 弹出框内容高度
                title: '任务设置',	// 弹出框标题
                url: 'TrustTaskConfig.html?id=' + id,
                onClose: function () {
                    //关闭的回调 list 的刷新方法
                    //searchByWhere();
                }
