define(function (require) {
    ...
    var ItemId = common.getQueryString("ItemId") ? common.getQueryString("ItemId") : "";
    var CategoryId = common.getQueryString("CategoryId") ? common.getQueryString("CategoryId") : "";
    ...
    function getMetaData() {
        ...
        var data = common.ExecuteGetData(false, config.svcUrl, 'TrustManagement', executeParam);
    ...
    function dicListFunction(obj, func) {
        if (func == 'add') {
            ...
            GSDialog.HintWindowTF("确定删除吗？删除后将无法使用", function () {
    ...
    function saveData() {
        ...
        if (config.DictionaryList.length > 0 && $("#itemDetailFrom select[data-attr='DataType']").val() == 'Select') {
            ...
            if (haveError) { GSDialog.HintWindow("下拉选项验证未通过，请重新编辑后保存"); return; };
        ...
        common.ExecuteGetData(true, config.svcUrl, 'TrustManagement', executeParam, function (data) {
            if (data[0].Result == 1) {
            ...
            else if (data[0].Result == 0) GSDialog.HintWindow('保存失败');
            else if (data[0].Result == 2) GSDialog.HintWindow('该ItemCode已存在');
        });
    }
    ...
    function AfterBindData() {
        IsRequiredChange($("#itemDetailFrom input[data-attr='IsRequired']"), $("#itemDetailFrom input[data-attr='IsRequired']").is(':checked'));
    ...
    $(function () {
        ...
        $("#itemDetailFrom input[data-attr='IsRequired']").change(function () {
            var $this = $(this);
            IsRequiredChange($this, $this.is(':checked'));
        })
