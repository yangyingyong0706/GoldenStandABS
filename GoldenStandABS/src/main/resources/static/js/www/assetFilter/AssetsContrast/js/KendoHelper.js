            ...
            var dataSourceOptions = {
                    ...
                    parameterMap: function (options, operation) {
                        ...
                        if (operation == "read") {
                            ...
                            if (options.filter && options.filter.filters.length > 0) {
                                ...
                                var filterSql;
                                if (options.filter.filters.length > 1) {
                                    filterSql = self.filterBuilder(options.filter)
                                ...
                                //$.each(filters, function (i, f) {
                                //    var filterSql;
                                //    if (f.filters && options.filter.logic) {
                                //        filterSql = self.filterBuilder(options.filter)
                                //    } else {
                                //        filterSql = self.getFilterSql(f.field, f.operator, f.value);
                                //    }
                            ...
                            callApi.AddParam({ Name: 'DimPoolId', Value: o.dataSource.params, DBType: 'int' });
                            callApi.AddParam({ Name: 'start', Value: start, DBType: 'int' });
                            callApi.AddParam({ Name: 'end', Value: end, DBType: 'int' });
                            callApi.AddParam({ Name: 'orderby', Value: orderby, DBType: 'string' });//direction
                            callApi.AddParam({ Name: 'direction', Value: direction, DBType: 'string' });//direction
                ...
                requestEnd: function (e) {
                    ...
                    if (result.resultCode == "1") {
                        window.top.location.href = gv.GoldenStandHostURL + '500.html';
                    }
                    else if (result.resultCode == "2") {
                        // alert("登录失效，点击确定后会跳转到登录页面。");
                        window.top.location.href = gv.GoldenStandHostURL + 'login/login.html?appDomain=' + common.appDomain();
                    }
                ...
                schema: {
                    ...
                    total: function (response) {
                        ...
                        return $.parseJSON(response).total;
                    }
                }
            };
            if (o.fieldsSchema) {
