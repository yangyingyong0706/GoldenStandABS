define(function (require) {
    ...
    $(function () {
        app = new vue({
            ...
            data: {
                ...
                IsAjustFirstTermPaymentRate: false,
                FirstPaymentRate: ''
            },
            watch: {
            	...
            	nonCyclingRateArr: {
            	    handler: function (newVal) {
            	        ...
            	        for (var i = 0; i < newVal.length; i++) {
            	            if (tex.test(newVal[i].rate)) {
            	                this.nonCyclingRateArr[i].rate=''
            	            }
            	        }
            	    },      
            	    deep: true    //深度监听
            ...
            methods: {
                inputBlur: function (event) {
                    ...
                    if (reg.test(target.val())) {
                        ...
                        target.val('');
                        target.attr('placeholder', "请输入数字")
                    } else {
                        target.removeClass('red_border');
                    }
                },
                dateInputBlur: function (event) {
                    var $target = $(event.target);
                    var v = $target.val();
                    if (v > 31 || v < 1) {
                        GSDialog.HintWindow('请输入1-31之间的整数！');
                        this.FirstPaymentRate = ''
                    } else {
                        this.FirstPaymentRate = parseFloat(v/30).toFixed(2);
                    }
                },
            	//打开历史记录页面
                ...
                addNonCyclingRate: function (){
                    var object = {
                        rate: ''
                    };
                    this.nonCyclingRateArr.push(object)
                },
                removeNonCyclingRate: function (index) {
                    this.nonCyclingRateArr.split(index,1)
                },
                addCyclingRate: function () {
                    var object = {
                        rate: ''
                    };
                    this.CyclingRateArr.push(object)
                },
                removeCyclingRate: function (index) {
                    this.CyclingRateArr.split(index, 1)
                },
            },
            filters: {
                //vue时间格式化
    ...
    function initReportDate() {
        ...
        var svcUrl = GlobalVariable.DataProcessServiceUrl + "CommonExecuteGet?";
        var executeParam = { SPName: 'dbo.usp_GetSelectedFactLoanDate', SQLParams: [] };
        ...
        executeParam = { SPName: 'dbo.usp_GetFactLoanDate', SQLParams: [] };
    ...
    function reisterEvent() {
        ...
        //全账户拆分 RunTask
        $("body").on("click", "#TaboneRunTask", function () {
            var nonCyclingRate = $('.nonCyclingRate');
            var CyclingRate = $('.CyclingRate');
            $.each(nonCyclingRate, function (i, v) {
                nonCyclingRateStr = nonCyclingRateStr + ($(v).val() ? $(v).val() : 0) + ",";
                if (i == nonCyclingRate.length - 1 && $(v).val() === "0") {
                    GSDialog.HintWindowTF('最后一期不能为0');
                    return;
                }
            })
            $.each(CyclingRate, function (i, v) {
                CyclingRateStr = CyclingRateStr + ($(v).val() ? $(v).val() : 0) + ",";
                if (i == CyclingRate.length - 1 && $(v).val() === "0") {
                    GSDialog.HintWindowTF('最后一期不能为0');
                    return;
                }
            })
            var xml = "<Coefficients><Coefficient Describe='非循环交易收益率' Name='NotCyclebeniProfitRate' Value='{0}' />"
                    + "<Coefficient Describe='循环交易收益率' Name='CyclebeniProfitRate' Value='{1}'/>"
                    + "<Coefficient Describe='非循环交易衰减速度' Name='NotCycleReduceSpeed' Value='{2}'/>"
                    + "<Coefficient Describe='循环交易衰减速度' Name='CycleReduceSpeed' Value='{3}'/>"
                    + "<Coefficient Describe='折扣系数' Name='DiscountCoefficient' Value='{4}'/></Coefficients>";
            var a = $("#NotCyclebeniProfitRate").val() == "" ? 0 : $("#NotCyclebeniProfitRate").val();
            var b = $("#CyclebeniProfitRate").val() == "" ? 0 : $("#CyclebeniProfitRate").val();
            var e = $("#DiscountCoefficient").val() == "" ? 0 : $("#DiscountCoefficient").val();
            var c = nonCyclingRateStr ? nonCyclingRateStr.substr(0, nonCyclingRateStr.length - 1) : ''
            var d = CyclingRateStr ? CyclingRateStr.substr(0, CyclingRateStr.length - 1) : ''
            xml = xml.StringFormat(a, b, c, d, e);
            var common = require('common');
            var PoolIds = common.getQueryString('PoolIds');
            var poolCutId = common.getQueryString('poolCutId');
            var taskIndicator = require('gs/taskProcessIndicator');
            sVariableBuilder.AddVariableItem('PoolIds', PoolIds, 'String', 1);
            sVariableBuilder.AddVariableItem('Coefficients', xml.replace(/</g, "(").replace(/>/g, ")"), 'Xml', 1);
            sVariableBuilder.AddVariableItem('ConnectionString', "Data Source=MSSQL;Initial Catalog=" + poolCutId + ";Integrated Security=SSPI;", 'String', 1);
        ...
        //摊还拆分 RunTask
        $("body").on("click", "#TabtwoRunTask", function () {
            var xml = "<Coefficients><Coefficient Describe='本金系数1' Name='PrincipalCoefficient1' Value='{0}'/>";
            xml += "<Coefficient Describe='本金系数2' Name='PrincipalCoefficient2' Value='{1}'/>";
            xml += "<Coefficient Describe='利息系数1' Name='InterestCoefficient1' Value='{2}'/>";
            xml += "<Coefficient Describe='利息系数2' Name='InterestCoefficient2' Value='{3}'/>";
            xml += "<Coefficient Describe='利息系数3' Name='InterestCoefficient3' Value='{4}'/>";
            xml += "</Coefficients>";
            var a = $("#PrincipalCoefficient1").val() == "" ? 0 : $("#PrincipalCoefficient1").val();
            var b = $("#PrincipalCoefficient2").val() == "" ? 0 : $("#PrincipalCoefficient2").val();
            var c = $("#InterestCoefficient1").val() == "" ? 0 : $("#InterestCoefficient1").val();
            var d = $("#InterestCoefficient2").val() == "" ? 0 : $("#InterestCoefficient2").val();
            var e = $("#InterestCoefficient3").val() == "" ? 0 : $("#InterestCoefficient3").val();
            xml = xml.StringFormat(a, b,c,d,e);
            var common = require('common');
            var PoolIds = common.getQueryString('PoolIds');
            var poolCutId = common.getQueryString('poolCutId');
        ...
        //汽车分期 RunTask
        $("body").on("click", "#TabThreeRunTask", function () {
            var common = require('common');
            var PoolIds = common.getQueryString('PoolIds');
            var poolCutId = common.getQueryString('poolCutId');
            var taskIndicator = require('gs/taskProcessIndicator');
            ...
            var tIndicator = new taskIndicator({
                ...
                clientName: 'TaskProcess',
                appDomain: 'Task',
                taskCode: 'AutoStagingAssetSplit',
                sContext: sVariable,
                callback: function () {
                    sVariableBuilder.ClearVariableItem();
                }
        ...
        $('#btnSubmit').click(function () {
            ...
            var CyclingRate = $('.CyclingRate');
            var off = false;
            if ($(nonCyclingRate.eq(nonCyclingRate.length - 1)).val() === "0" || $(CyclingRate.eq(CyclingRate.length - 1)).val() === "0") {
                GSDialog.HintWindowTF('最后一期不能为0');
                return;
            }
            var nonCyclingRateStr = '', CyclingRateStr = '';
            $.each(nonCyclingRate, function (i, v) {
                if (!$(v).val()) {
                    off = true;
                }
            ...
            if (off) {
                GSDialog.HintWindowTF('衰减速度不能为空！');
                return;
            }
            if (app.IsAjustFirstTermPaymentRate) {
                var xml = "<Coefficients>"
                        + "<ThrowAccountSplit>"
                        + "<Coefficient Describe='本金系数1' Name='PrincipalCoefficient1' Value='{3}' />"
                        + "<Coefficient Describe='本金系数2' Name='PrincipalCoefficient2' Value='{4}' />"
                        + "<Coefficient Describe='利息系数1' Name='InterestCoefficient1' Value='{5}' />"
                        + "<Coefficient Describe='利息系数2' Name='InterestCoefficient2' Value='{6}' />"
                        + "<Coefficient Describe='利息系数3' Name='InterestCoefficient3' Value='{7}' />"
                        + "<Coefficient Describe='调整首期还款比例' Name='IsAjustFirstTermPaymentRate' Value='1' />"
                        + "<Coefficient Describe='首期还款比例' Name='FirstPaymentRate' Value='{8}' />"
                        + "<Coefficient Describe='循环购买日' Name='RevolvingDay' Value='{9}' />"
                        + "</ThrowAccountSplit>"
                        + "<AmortizationSplit>"
                        + "<Coefficient Describe='非循环交易衰减速度' Name='NotCycleReduceSpeed' Value='{0}'/>"
                        + "<Coefficient Describe='循环交易衰减速度' Name='CycleReduceSpeed' Value='{1}'/>"
                        + "<Coefficient Describe='折扣系数' Name='DiscountCoefficient' Value='{2}'/>"
                        + "</AmortizationSplit>"
                    + "</Coefficients>";
                var a = nonCyclingRateStr ? nonCyclingRateStr.substr(0, nonCyclingRateStr.length - 1) : ''
                var b = CyclingRateStr ? CyclingRateStr.substr(0, CyclingRateStr.length - 1) : '';
                var c = $("#DiscountCoefficient").val() == "" ? 0 : $("#DiscountCoefficient").val();
                var d = $("#PrincipalCoefficient1").val() == "" ? 0 : $("#PrincipalCoefficient1").val();
                var e = $("#PrincipalCoefficient2").val() == "" ? 0 : $("#PrincipalCoefficient2").val();
                var f = $("#InterestCoefficient1").val() == "" ? 0 : $("#InterestCoefficient1").val();
                var g = $("#InterestCoefficient2").val() == "" ? 0 : $("#InterestCoefficient2").val();
                var h = $("#InterestCoefficient3").val() == "" ? 0 : $("#InterestCoefficient3").val();
                var i = $("#FirstPaymentRate").val() == "" ? 0 : $("#FirstPaymentRate").val();
                var j = $("#RevolvingDay").val() == "" ? 0 : $("#RevolvingDay").val();
                xml = xml.StringFormat(a, b, c, d, e, f, g, h, i, j);
            } else {
                var xml = "<Coefficients>"
                        + "<ThrowAccountSplit>"
                        + "<Coefficient Describe='本金系数1' Name='PrincipalCoefficient1' Value='{3}' />"
                        ...
                        + "<Coefficient Describe='利息系数2' Name='InterestCoefficient2' Value='{6}' />"
                        + "<Coefficient Describe='利息系数3' Name='InterestCoefficient3' Value='{7}' />"
                        + "<Coefficient Describe='调整首期还款比例' Name='IsAjustFirstTermPaymentRate' Value='0' />"
                        + "</ThrowAccountSplit>"
                        + "<AmortizationSplit>"
                        + "<Coefficient Describe='非循环交易衰减速度' Name='NotCycleReduceSpeed' Value='{0}'/>"
                        + "<Coefficient Describe='循环交易衰减速度' Name='CycleReduceSpeed' Value='{1}'/>"
                ...
                var h = $("#InterestCoefficient3").val() == "" ? 0 : $("#InterestCoefficient3").val();
                var i = $("#FirstPaymentRate").val() == "" ? 0 : $("#FirstPaymentRate").val();
                xml = xml.StringFormat(a, b, c, d, e, f, g, h, i);
            }
            ...
            sVariableBuilder.AddVariableItem('DimReportingDateId', data.ReportingDate.replace(/-/g, ''), 'Int', 1, 0, 0);
            ...
            //抛帐拆分 摊还拆分
            sVariableBuilder.AddVariableItem('Coefficients', xml.replace(/</g, "(").replace(/>/g, ")"), 'Xml', 1);
            sVariableBuilder.AddVariableItem('PoolDBName', poolCutId, 'String', 1);
            sVariableBuilder.AddVariableItem('PoolDBConnectionStr', "Data Source=MSSQL;Initial Catalog=" + poolCutId + ";Integrated Security=SSPI;", 'String', 1);
            var sVariable = sVariableBuilder.BuildVariables();
            ...
            var tIndicator = new taskIndicator({
                ...
                clientName: 'TaskProcess',
                appDomain: 'Task',
                taskCode: 'ImportTrustAssetByFactLoanForPoolCut',//Call new ImportTrustAssetByFactLoan task
                sContext: sVariable,
                callback: function () {
        ...
        $('.splitRuleOptionList>li').click(function () {
            ...
            var calculateInterestByDaysV = CalculateInterestByDays.css('display') == 'block' ? $("#CalculateInterestByDays").val() : 0; selectValText
            //if (currentRuleNo == 4) {
            //    var isBaseOnLoanTermV = IsBaseOnLoanTerm.css('display') == 'block' ? $("#IsBaseOnLoanTerm").val(): 0;
    ...
    function saveCashFlowSplitInfo(TrustId, ReadDate, ScheduleDate, Step, H, E, callback) {
        ...
        var svcUrls = webProxy.dataProcessServiceUrl + "CommonGetExecute?connConfig=TrustManagement&appDomain=" + poolCutId + "&executeParams=";
        var promise = webProxy.comGetData(parameterDatas, svcUrls, 'TrustManagement.usp_NewSaveCashFlowSplitInfo');
    ...
    function getStepInfo(TrustId, ReadDate, ScheduleDate, callback) {
        if (rk == 3) {
            ...
            var svcUrls = webProxy.dataProcessServiceUrl + "CommonGetExecute?connConfig=TrustManagement&appDomain=" + poolCutId + "&executeParams=";
            var promise = webProxy.comGetData(parameterDatas, svcUrls, 'TrustManagement.usp_getStepInfo_New');
            promise().then(function (response) {
    ...
    //显示新规则盒子
    function newRulebox(name) {
        switch (name) {
            case '摊还拆分':
                $("#tabone").show();
                $("#tabtwo").hide();
                showRule(false, false, false, false, false, false, false, false);
                break;
            case '抛账拆分':
                $("#tabone").hide();
                $("#tabtwo").show();
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.SaveTemplateOfCashflowSplitRulesResult == 1) {
                        ...
                        $.toast({
                            type: 'warning', message: '配置名称重复', afterHidden: function () {
                                $("#ConfigCode").val("")
                                LoadSplitList()
                            }
                        })
    ...
    function LoadSplitList() {
        ...
        promise().then(function (response) {
            ...
            $.each(ConfigJson, function (i, v) {
                html += '<tr><td><div style="padding:10px"><i class="icon icon-cog-alt" title="加载配置" style="color:#5767A6;cursor:pointer;margin: 0px 10px;"></i>'
    ...
    function LoadSplitConfig(parmas) {
        ...
        var parameterDatas = [
            ...
            ["UserName", UserName ? UserName : "goldenStand", "string"]
        ]
        var svcUrls = webProxy.dataProcessServiceUrl + "CommonGetExecute?connConfig=TrustManagement&appDomain=TrustManagement&executeParams=";
        var promise = webProxy.comGetData(parameterDatas, svcUrls, 'usp_GetCashflowSplitHistory');
        promise().then(function (response) {
