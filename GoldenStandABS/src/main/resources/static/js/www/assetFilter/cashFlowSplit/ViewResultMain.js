        ...
        vm = new Vue({
            ...
            methods: {
                ...
                confirmCombo: function () {//选定
                    ...
                    function startwork() {
                        executeParam = {
                            SPName: 'TrustManagement.usp_RegisterCurrentPaymentSchedule', SQLParams: [
                                {
                        ...
                        common.ExecuteGetData(false, svcUrl, poolCutId, executeParam, function () {
                    ...
                    if (schedulePurpose == 0) {
                        executeParam = {
                            SPName: 'TrustManagement.usp_ComfirmPaymentScheduleData', SQLParams: [
                                { Name: 'TrustId', value: TrustId, DBType: 'int' }
                            ]
                        };
                        common.ExecuteGetData(false, svcUrl, poolCutId, executeParam, function (data) {
                ...
                downloadExcel: function () {
                    var isCheck = vm.hchartCombo.length > 0;
                    if (vm.hchartCombo.length < 1) {
                        GSDialog.HintWindow('未选择任何资产池，无法导出。');
                        return;
                    }
                    var poolList = ''; //组织资产列表xml
                    if (vm.chkOptions.length > 0) {
                        ...
                        for (var i = 0; i < vm.chkOptions.length; i++) {
                            //vm.hchartCombo.length
                            for (var j = 0; j < vm.hchartCombo.length; j++) {
                                if (vm.chkOptions[i].id === vm.hchartCombo[j]) {
                                    if (vm.chkOptions[i].type == '资产池') {
                                        poolList += '<Pool><Id>{0}</Id><DimReportingDateId>{1}</DimReportingDateId><Purpose>{2}</Purpose></Pool>'.format(vm.chkOptions[i].id, vm.chkOptions[i].dimreportingdateid, vm.chkOptions[i].sel);
                                    }
                                    else {
                                        //type=='基础池'
                                        if (poolList.indexOf('-1') < 0) {
                                            //不存在基础池
                                            poolList += '<Pool><Id>-1</Id><DimReportingDateId>{0}</DimReportingDateId><Purpose>{1}</Purpose></Pool>'.format(reportingDateId, vm.chkOptions[i].sel);
                                        }
                                    }
                                    break;
                                }
                    ...
                    var callApi = new CallApi(poolCutId, 'Asset.usp_getCashflowCollection_Purpose', true);
                    callApi.AddParam({ Name: 'TrustId', Value: TrustId, DBType: 'int' });
                    callApi.AddParam({ Name: 'PoolList', Value: poolList, DBType: 'xml' });
                    callApi.AddParam({ Name: 'SchedulePurpose', Value: schedulePurpose, DBType: 'int' });
                    callApi.AddParam({ Name: 'ScheduleDateId', Value: scheduleDateId, DBType: 'int' });
