    ...
    TRUST = {
        ...
        init: function () {
            ...
            config.$next.click(function (e) {
                ...
                var isPass = true, temp;
                var len = $(".aside_list_box").find("li:visible").length;
                var isCycle = [];
                var isAmortization = [];
                var isAmortizationCount = [];
                var isCycleCount = [];
                if ($(e.currentTarget).hasClass("date_set_button") && len == 3) {
                    //var $Null = [];
                    //$Null.push($('.red-border'));
                    //var isCycle = $Null[0].isChildOf("#Cycle_period");
                    //var isAmortization = $Null[0].isChildOf("#Amortization_period");
                    $(".FirstDate").each(function (i,v) {
                        if ($(v).text() == "") {
                            if ($(v).parents(".table")[0].id == "c_table") {
                                isCycle.push(true)
                            } else {
                                isAmortization.push(true)
                            }
                        }
                    })
                    $(".Frequency").each(function (i, v) {
                        if ($(v).text() == "") {
                            if ($(v).parents(".table")[0].id == "c_table") {
                                isCycle.push(true)
                            } else {
                    ...
                    $(".DateCount").each(function (i, v) {
                        if ($(v).val() == "") {
                            if ($(v).parents("tbody")[0].id == "Amortization_period_table") {
                                isAmortizationCount.push(true)
                            } else {
                                isCycleCount.push(true)
                            }
                        }
                    })
                    if (isCycle.indexOf(true) > -1) {
                        GSDialog.HintWindow('循环期数据不能为空!');
                        return;
                    } else if (isAmortization.indexOf(true) > -1) {
                        GSDialog.HintWindow('摊还期数据不能为空!');
                        return;
                    } else if (isAmortization.indexOf(true) > -1 && isCycle.indexOf(true) > -1) {
                        GSDialog.HintWindow('循环期摊还期数据不能为空!');
                        return;
                    }
                    if (isCycleCount.indexOf(true) > -1) {
                        GSDialog.HintWindow('循环期计算日不能为空!');
                        return;
                    } else if (isAmortizationCount.indexOf(true) > -1) {
                        GSDialog.HintWindow('摊还期计算日不能为空!');
                        return;
                    }
                } else {
                    $(".FirstDate:visible").each(function (i, v) {
                        if ($(v).text() == "") {
                            isAmortization.push(true)
                        }
                    })
                    $(".Frequency:visible").each(function (i, v) {
                        if ($(v).text() == "") {
                            isAmortization.push(true)
                        }
                    })
                    $(".DateCount:visible").each(function (i, v) {
                        if ($(v).val() == "") {
                            isAmortizationCount.push(true)
                        }
                    })
                    if (isAmortization.indexOf(true) > -1) {
                        GSDialog.HintWindow('摊还期数据不能为空!');
                        return;
                    } else if (isAmortizationCount.indexOf(true) > -1) {
                        GSDialog.HintWindow('摊还期计算日不能为空!');
                ...
                if (self.stepId == 3) {
                    ...
                    if (!flag) {
                        GSDialog.HintWindow('产品已发行状态不允许修改日期！');
                       return false
                    }
                    //日期就相当于是原来的stepId=3
                    ...
                    self.saveWorkingSessionContext(sessionContext, function (sessionId) {
                        ...
                        var tIndicator = new taskIndicator({
                            ...
                            callback: function () {
                                ...
                                var SessionId = webStorage.getItem('sessionId');
                                //var executeParam = { SPName: 'Task.usp_GetSession', SQLParams: [{ Name: 'SessionId', Value: SessionId, DBType: 'uniqueidentifier' }] };
                                //var executeParams = encodeURIComponent(JSON.stringify(executeParam));
                                //var serviceUrl = GlobalVariable.PoolCutServiceURL + 'CommonGet?connName=TaskProcess&exeParams=' + executeParams;
                                var svcUrl = config.tmsDataProcessBase + "CommonGetExecuteForPool?poolname=TaskProcess&";
                               // var sessionContext = '', sessionContextArray = [];
                                var executeParam = {
                                    SPName: 'usp_GetSession', SQLParams: [
                                        { Name: 'SessionId', Value: SessionId, DBType: 'string' }
                                    ]
                                }
                                common.ExecuteGetData(false, svcUrl, 'Task', executeParam, function (response) {
                                    //console.log(response);
                                    if (response[0].SessionStatus == "Completed") {
        ...
        getTrustInfoByTrustId: function () {
           ...
           /* var sContent = "{'SPName':'usp_GetTrustInfoFromWizard','Params':{" +
                            ...
                            "'}}";*/
            trustId="3627";
            //TODO YANGYINIGYONG  修改成 url可以傳入的信息值
/*            var sContent = {
                    'SPName': "usp_GetTrustInfoFromWizard", 'SQLParams': [
...
/*            var sContent = "{'SPName':'usp_GetTrustInfoFromWizard','SQLParams':[{" +
            "'Name':'TrustId','Value':'" + trustId +"','DBType':'int'}]}";
*/            
...
//	                		alert(obj[key]);//可以输出值信息
	                		console.log("GetItemsPlus?applicationDomain=TrustManagement&contextInfo----key:"+key+"--val:"+val);//可以输出姓名
	                		});
                	 }*/
                    //callBack(response);
                    //TrustInfo = response;
        ...
        getAllCodeDictionary: function () {
            var self = this;
//            var sContent = "{\"SPName\":\"usp_GetAllCodeDictionary\",\"Params\":{\"AliasSetName\":\"zh-CN\"}}";
            //TODO YANGYINGYONG 提交信息修改成json格式
/*            var sContent = {
                    'SPName': "usp_GetAllCodeDictionary", 'Params': [
                        {'AliasSetName': 'zh-CN'}
                    ]};
            */
