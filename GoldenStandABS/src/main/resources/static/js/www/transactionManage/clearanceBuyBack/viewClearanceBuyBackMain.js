...
define(function (require) {
    ...
    if (userLanguage && userLanguage.indexOf('en') > -1) {
        ...
        lang.date = 'Are sure to recycle data?';
    }
    ...
    AssetTransferMain = (function () {
        ...
        $('#selectLanguageDropdown_viewClearanceBuyBack').localizationTool({
            ...
            'strings': {
                ...
                'class:viewClearanceBuyBack_ClearanceBuyBackDate': {
                    'en_GB': 'Redemption Date: '
                },

                'class:viewClearanceBuyBack_ClearanceBuyBack': {
                    'en_GB': 'Redemption'
                }
        ...
        var userLanguage = webStorage.getItem('userLanguage');
        if (userLanguage) {
            $('#selectLanguageDropdown_viewClearanceBuyBack').localizationTool('translate', userLanguage);
        }
        $('body').show();
            ...
            function InitKendoGrid(data) {  //检查用户是否是管理员
                ...
                kendouiGrid.Init({
                    renderOptions: {
                        ...
                        scrollable: true,
                        columns: [
                                { selectable: false, width: "50px", template: "<input name='AccountNoCkbox' class='ob-paid' type='checkbox' data-bind='checked: AccountNo' #= AccountNo ? checked='checked' : '' #/>" }
