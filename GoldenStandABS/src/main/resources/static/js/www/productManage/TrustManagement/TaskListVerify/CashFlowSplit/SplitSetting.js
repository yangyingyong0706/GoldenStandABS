                     ...
                     var tIndicator = new taskIndicator({
                         ...
                         callback: function () {
                             ...
                             webProxy.getSessionProcessStatusList(sessionId, "ConsumerLoan", function (response) {
                                 for (let i = 0; i < response.GetSessionProcessStatusListResult.length; i++) {
                                     if (response.GetSessionProcessStatusListResult[i].ActionStatus != "Success") {
    ...
    function reisterEvent() {
        ...
        /*
         ...
         $("body").on("click", ".maketrue", function () {
             ...
             if (newValue == "") {
                 GSDialog.HintWindow("请填写名称！", function () { }, false)
                 return false
             }
             var executeParam = {
             ...
             common.ExecuteGetData(false, svcUrl, 'TrustManagement', executeParam, function (data) {
                 if (data[0].result == '0') {
                     GSDialog.HintWindow("配置名称重复！", function () { }, false)
                 } else if (data[0].result == '1') {
                     GSDialog.HintWindow("修改配置名称成功！", function () { }, false)
                     LoadSplitList();
                 }
             })
         })
         ...
         $("body").on("click", ".icon.icon-trash-empty", function () {
             ...
             GSDialog.HintWindowTF('是否确认删除该配置?', function () {
                 ...
                 common.ExecuteGetData(false, svcUrl, 'TrustManagement', executeParam, function (data) {
                     GSDialog.HintWindow('删除成功', "", false)
                     LoadSplitList();
                 })
             }, '', false)
        ...
        $('#btnSubmit').click(function () {
            ...
            sVariableBuilder.AddVariableItem('IsTask', IsTask, 'String', 0, 0, 0);
