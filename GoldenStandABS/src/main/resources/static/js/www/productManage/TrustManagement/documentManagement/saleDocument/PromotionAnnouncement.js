    ...
    new Vue({
        ...
        methods: {
            ...
            HistoricalRecords: function () {
                ...
                executeParam.SQLParams.push({ Name: 'TrustId', value: that.trustId, DBType: 'int' });
                executeParam.SQLParams.push({ Name: 'DoucumentType', value: 'PromotionAnnouncement', DBType: 'string' });
                var executeParams = encodeURIComponent(JSON.stringify(executeParam));
            ...
            getLayeredContent: function () {
                ...
                var trustId = common.getQueryString("trustId")
                var svcUrlWithConn = GlobalVariable.DataProcessServiceUrl + "CommonGetExecute?";
                var executeParam = { SPName: 'usp_GetSalesPopularizeDocumentInfo', SQLParams: [] };
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < data[0].length; i++) {
                            if (data[0][i].name == "TrustName") {
                                self.EnterpirseName = data[0][i].value;
                            }
                            if (data[0][i].name == "PlanManagerAdvertiser") {
                                self.Planner = data[0][i].value;
                            }
                            if (data[0][i].name == "TrusteeBank") {
                                self.TrusteeshipBank = data[0][i].value;
                            }
                            if (data[0][i].name == "PopularizeDate") {
                                $('#pullDate').val(data[0][1].value)
                            }
                        }
                        for (var i = 0; i < data[1].length; i++) {
                            if (data[1][i].PlanManagerAdvertiserInfo == "FirstContact") {
                                self.Contacts = data[1][i].PlanManagerAdvertiserValue;
                            }
                            if (data[1][i].PlanManagerAdvertiserInfo == "Web") {
                                self.PlannerWebsite = data[1][i].PlanManagerAdvertiserValue;
                            }
                            if (data[1][i].PlanManagerAdvertiserInfo == "Phone") {
                                self.PlannerPhone = data[1][i].PlanManagerAdvertiserValue;
                            }
                            if (data[1][i].PlanManagerAdvertiserInfo == "Email") {
                                self.PlannerEmail = data[1][i].PlanManagerAdvertiserValue;
                            }
                        }
                    },
                    error: function (data) {
            ...
            saveAllItems: function () {
                ...
                var checktel = checkTel(this.PlannerPhone)
                if (!checktel) {
                    $('.tel').addClass('red-border')
                    return
                } else {
                    $('.tel').removeClass('red-border')
                }
                var checkEmail = isEmail(this.PlannerEmail)
                if (!checkEmail) {
                ...
                var xml = '<xml><PopularizeDate>' + $("#pullDate").val() + '</PopularizeDate>' +
                    '<PlanManagerAdvertiser>' + this.Planner + '</PlanManagerAdvertiser>' +
                    '<TrusteeBank>' + this.TrusteeshipBank + '</TrusteeBank>' +
                '<PlanManagerAdvertiserInfo>' +
                    '<Web>' + this.PlannerWebsite + '</Web>' +
                    '<Email>' + this.PlannerEmail + '</Email>' +
                    '<Phone>' + this.PlannerPhone + '</Phone>' +
                    '<FirstContact>' + this.Contacts + '</FirstContact>' +
                '</PlanManagerAdvertiserInfo></xml>'

                var trustId = common.getQueryString("trustId")
                var executeParam = {
                    SPName: 'usp_SaveSalesPopularizeDocumentInfo', SQLParams: [
