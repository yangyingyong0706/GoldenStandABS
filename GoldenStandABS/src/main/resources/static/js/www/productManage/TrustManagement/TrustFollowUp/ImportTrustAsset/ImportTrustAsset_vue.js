define(function (require) {
    ...
    var modes = [
        {
            CalculateHeadInterestRule: "0"
            , CalculateTailInterestRule: 0
            , CalculateTimeInterestRule: 0
            , IsBaseOnLoanTerm: "0"
            , AdjustLastPaymentBalance: 0
            , AdjustBalaceDirection: 1
            , AdjustBalacePosition: 1
            , ModeName: "等额本息"
            , ModeNo: 0
            , TrailPeriodsCalRule: 0
        },
         {
             CalculateHeadInterestRule: "0"
            , CalculateTailInterestRule: 0
         ...
         },
          {
          ...
          },
           {
            ...
            , AdjustBalaceDirection: 1
            , AdjustBalacePosition: 1
            , ModeName: "按期付息,到期一次性还本"
            , ModeNo: 4
            , TrailPeriodsCalRule: 0
            },
             {
                 CalculateHeadInterestRule: 1
            , CalculateTailInterestRule: 0
            , CalculateTimeInterestRule: 0
            , IsBaseOnLoanTerm: "0"
            , AdjustLastPaymentBalance: 0
    ...
    function clickSubmitOption() {
        $('#btnSubmitOption').click(function () {
            var $this = $(this);
            splitRuleOptionLi.trigger("click");
            $('#more-options').slideToggle(200, function () {
                $(".btnsList").css({ "top": "auto", "left": "110", "padding": "0" })
            });
        });
    }
    $(function () {
        var app = new vue({
            ...
            components: {
                "navComp0": {
                    ...
                    methods: {
                        assetAttributesNavClick: function (index) {
                            this.assetAttributesNavIndex = index;
                        },
                    },
                },
                "navComp1": {
            ...
            methods: {
                navClick: function (index) {
                    this.navFuncIndex = index;
                    this.currentView = 'navComp' + index; // 切换currentView
                },
        ...
        //拆分规则配置
        var splitRuleOptionList = $(".splitRuleOptionList>li");
        var assetAttributesOptionList = $(".assetAttributesOptionList>li");
        var CollectionRuleOptionList = $(".CollectionRuleOptionList>li");
        var IsBaseOnLoanTerm = $("#IsBaseOnLoanTerm").parent();
        var CalculateHeadInterestByDay = $("#CalculateHeadInterestByDay").parent();
        var CalculateInterestByDays = $("#CalculateInterestByDays").parent();
        var CountLastInterestPeriodByDay = $("#CountLastInterestPeriodByDay").parent();
        var AdjustLastPaymentBalance = $("#AdjustLastPaymentBalance").parent();
        var AdjustBalaceDirection = $("#AdjustBalaceDirection").parent();
        var AdjustBalacePosition = $("#AdjustBalacePosition").parent();
        var TrailPeriodsCalRule = $("#TrailPeriodsCalRule").parent();
        //现金流拆分初始化显示
        IsBaseOnLoanTerm.show();
        CalculateHeadInterestByDay.show();
        //归集规则配置
        var IsBasedOnReportingDate = $("#IsBasedOnReportingDate").parent();
        var IsRidPrepaid = $("#IsRidPrepaid").parent();
        var IsNormalAsset = $("#IsNormalAsset").parent();
        IsBasedOnReportingDate.show();
        //资产属性计算
        var PeriodsCalRule = $("#PeriodsCalRule").parent();
        var CalculateRTBySystem = $("#CalculateRTBySystem").parent();
        var RTCalculationRuleOptions = $('#RTCalculationRule-Options');
        CalculateRTBySystem.show();

        //(isBaseOnLoanTermV, calculateHeadInterestByDayV, trailPeriodsCalRuleV, calculateInterestByDaysV, countLastInterestPeriodByDayV, repaymentModeNo
        //通用的规则组合方法
        ...
        function selectmode(
            ...
            //$("#TrailPeriodsCalRule").val(), $("#CalculateInterestByDays").val() CountLastInterestPeriodByDay
            var isBaseOnLoanTermV = isBaseOnLoanTerm ? $("#IsBaseOnLoanTerm").val() : 0;
            var calculateHeadInterestByDayV = calculateHeadInterestByDay ? $("#CalculateHeadInterestByDay").val() : 0;
            var trailPeriodsCalRuleV = trailPeriodsCalRule ? $("#TrailPeriodsCalRule").val() : 0;
            var calculateInterestByDaysV = calculateInterestByDays ? $("#CalculateInterestByDays").val() : 0;
            var countLastInterestPeriodByDayV = countLastInterestPeriodByDay ? $("#CountLastInterestPeriodByDay").val() : 0;
            var adjustLastPaymentBalanceV = adjustLastPaymentBalance ? $("#AdjustLastPaymentBalance").val() : 0;
            var adjustBalaceDirectionV = adjustBalaceDirection ? $("#AdjustBalaceDirection").val() : 0;
            var adjustBalacePositionV = adjustBalacePosition ? $("#AdjustBalacePosition").val() : 0


            selectValText(isBaseOnLoanTermV, calculateHeadInterestByDayV, trailPeriodsCalRuleV, calculateInterestByDaysV, countLastInterestPeriodByDayV, repaymentModeNo, function (res) {
                getAssetSplitRuleDataCalback(res)
            });

            var rulesInMode = {};
            rulesInMode.ModeName = repaymentModeName;
            rulesInMode.ModeNo = repaymentModeNo;
            ...
            modes[repaymentModeNo] = rulesInMode;
        ...
        //拆分规则分类、配置
        for (var i = 0; i < splitRuleOptionList.length; i++) {
            (function (i) {
                splitRuleOptionList[i].onclick = function () {
                    ...
                    switch (thisVal) {
                        case "等额本息":
                            var RepaymentMode = '0';
                            selectmode(true, true, false, false, false, false, false, false, 0, thisVal);
                            //改变select的值后重新选择拆分规则
                            $("#IsBaseOnLoanTerm").removeClass();
                            $("#CalculateHeadInterestByDay").removeClass();
                            $("#IsBaseOnLoanTerm").addClass('mode0');
                            $("#CalculateHeadInterestByDay").addClass('mode0');
                            $(".mode0").change(function () {
                                //selectValText($("#IsBaseOnLoanTerm").val(), $("#CalculateHeadInterestByDay").val(), null, null, null, RepaymentMode, function (res) {
                                //    getAssetSplitRuleDataCalback(res)
                                //});

                                if ($("#IsBaseOnLoanTerm").val() == 1) {
                                    selectmode(true, false, false, false, false, false, false, false, 0, thisVal);
                                }
                                else {
                                    selectmode(true, true, false, false, false, false, false, false, 0, thisVal);
                                }
                            ...
                            //$("#IsBaseOnLoanTerm").change(function () {
                            //    selectValText($("#IsBaseOnLoanTerm").val(), $("#CalculateHeadInterestByDay").val(), null, null, null, RepaymentMode, function (res) {
                            //        getAssetSplitRuleDataCalback(res)
                            //    });
                            //});
                            //$("#CalculateHeadInterestByDay").change(function () {
                            //    selectValText($("#IsBaseOnLoanTerm").val(), $("#CalculateHeadInterestByDay").val(), null, null, null, RepaymentMode, function (res) {
                            //        getAssetSplitRuleDataCalback(res)
                            //    });
                            //});
                        ...
                        case "等额本金":
                            var RepaymentMode = '1';
                            selectmode(true, true, true, true, false, false, false, false, 1, thisVal);
                            $("#IsBaseOnLoanTerm").change(function () {
                                selectmode(true, true, true, true, false, false, false, false, 1, thisVal);
                            });
                            $("#CalculateHeadInterestByDay").change(function () {
                            ...
                            $("#TrailPeriodsCalRule").change(function () {
                                selectmode(true, true, true, true, false, false, false, false, 1, thisVal);
                            });
                            $("#CalculateInterestByDays").change(function () {
                                selectmode(true, true, true, true, false, false, false, false, 1, thisVal);
                            });
                            break;
                        case "等本等息":
                            ...
                            selectmode(true, true, true, false, true, true, true, true, 2, thisVal);
                            $("#IsBaseOnLoanTerm").change(function () {
                                selectmode(true, true, true, false, true, true, true, true, 2, thisVal);
                            });
                            $("#CalculateHeadInterestByDay").change(function () {
                        ...
                        case "按计划还款":
                            ...
                            selectmode(false, true, false, true, false, false, false, false, 3, thisVal);
                            $("#CalculateInterestByDays").change(function () {
                                selectmode(false, true, false, true, false, false, false, false, 3, thisVal);
                            });
                            $("#CalculateHeadInterestByDay").change(function () {
                        ...
                        case "按期付息,到期一次性还本":
                            ...
                            selectmode(true, true, false, true, false, false, false, false, 4, thisVal);
                        ...
                        case "到期一次性还本付息":
                            ...
                            selectmode(true, false, false, false, false, false, false, false, 5, thisVal);
                            $("#IsBaseOnLoanTerm").change(function () {
                                selectmode(true, false, false, false, false, false, false, false, 5, thisVal);
                            });
                            break;
    ...
    function reisterEvent() {
        ...
        $('#btnSubmit').click(function () {
            ...
            sVariableBuilder.AddVariableItem('ScheduleDateId', scheduleDate.replace(/-/g, ''), 'String', 1);
            sVariableBuilder.AddVariableItem('ScheduleDate', scheduleDate, 'String', 1);
