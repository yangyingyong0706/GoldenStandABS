define(['jquery', 'globalVariable', 'common', 'gsAdminPages', 'Vue2', 'echarts', 'taskProcessIndicator', 'sVariableBuilder', 'highcharts', 'highchartsexporting', 'kendomessagescn', 'kendoculturezhCN', 'macarons'], function ($, GlobalVariable, common, GSDialog, Vue, echarts, taskIndicator, sVariableBuilder) {
    ...
    var Vue = new Vue({
        ...
        data: {
            ...
            intervalStatisticsExecuteParams: {
                SPName: 'usp_GetTrustBackedOverViewByDate', SQLParams: [
        ...
        mounted: function () {
            ...
            self.getArrears();
            var statisicstype = $("#statisicstype").val();
            if (statisicstype != null && statisicstype != "") {
               /** var self = this;
                typeState = 2;
                self.Startyear = true;
                self.Endyear = false;
                self.RenderView('2', null, null);**/
            } else {
                self.RenderView('1', null, null);
            }
            $("#loading").hide();
        ...
        methods: {
            ...
            GetPeriodData: function () {
                var self = this;
                var executeParam = {
                    SPName: 'usp_GetTrustPeriodNew', SQLParams: [
            ...
            //渲染数据图表
            RenderView: function (typeState, StartDate, EndDate) {
                var self = this;
                var bool = true;
                if (EndDate == null) {
                    ...
                    executeParamsDate.SQLParams.push({ Name: 'type ', value: typeState, DBType: 'string' });
                ...
                var executeParams = {
                    SPName: 'usp_AssetPoolTemporaryNew', SQLParams: [
                        { Name: 'TrustId', value: trustId, DBType: 'int' }
                    ]
                }
                //debugger
                executeParams.SQLParams.push({ Name: 'type ', value: typeState, DBType: 'string' });
            ...
            RenderTables: function (parmas) {
                ...
                AmountDiff_Avg = AccountTotal != 0 ? AmountDiff_Total / AccountTotal : 0;
            ...
            changeSelect: function () {
                ...
                self.Startyear = true;
                self.Endyear = false;
                self.statiscsTIndicatoropen();
                self.intervalStatisticsExecuteParams.SQLParams.splice(0, self.intervalStatisticsExecuteParams.SQLParams.length);
                self.intervalStatisticsExecuteParams.SQLParams.push({ Name: 'trustid', value: trustId, DBType: 'int' });
                self.intervalStatisticsExecuteParams.SQLParams.push({ Name: 'startdate ', value: self.StartDate, DBType: 'string' });
                self.intervalStatisticsExecuteParams.SQLParams.push({ Name: 'enddate ', value: self.EndDate, DBType: 'string' });
                common.ExecuteGetData(false, svcUrl, "TrustManagement", self.intervalStatisticsExecuteParams, function (data) {
                    self.tableDatas = [];
                    $.each(data, function (i, v) {

                        self.tableDatas.push(v);
            ...
            showList: function (params, mode1) {
                ...
                $.anyDialog({
                    title: title,
                    url: './statisticsView.html?tid=' + trustId + '&&Type=' + Type + "&&mode=" + mode + "&&startDate=" + startDate + "&&endDate=" + endDate,
                    width: '900',
                    height: '500',
                    draggable: false,
                    changeallow: false,
            ...
            statiscsTIndicatoropen: function () {//点击查询触发事件
                ...
                if (self.Endyear) {
                    ...
                    self.StartDate = startdate;
                    self.EndDate = enddate;
                    self.fontinfo = "从 " + startdate + " 到 " + enddate;
                    self.fontinfo2 = "截止到 " + enddate;
                ...
                sVariableBuilder.AddVariableItem('type', typeState, 'String', 0, 0, 0);
                ...
                var tIndicator = new taskIndicator({
                    ...
                    appDomain: 'Task',
                    taskCode: 'statisticsReportAllNew',
                    sContext: sVariable,
                    callback: function () {
                        self.RenderView(typeState, startdate, enddate);
