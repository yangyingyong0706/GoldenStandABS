...
// json data
define(['jquery', 'globalVariable', 'common', "gsAdminPages"], function ($, GlobalVariable, common, GSDialog) {
    var TrustInfo, trustId, OptionSource, idList;

    var tid = common.getQueryString("tid") ? common.getQueryString("tid") : "";
    var RDate = common.getQueryString("RDate") ? common.getQueryString("RDate") : "";
    var OriginatorId = common.getQueryString("oid") ? common.getQueryString("oid") : "";
    ...
    var TrustFllowUp = {
        ...
        init: function () {
            ...
            config.$next.click(function () {
                ...
                if (!canSubmit) { GSDialog.HintWindow('页面存在输入错误，无法提交！'); return; }
                // 上传数据
        ...
        saveWorkingSessionContext: function (sessionContext, callback) {
            ...
            var serviceUrl = config.tmsDataProcessBase + "CommonExecuteGet" +
                "?appDomain={0}&executeParams={1}&&resultType=com".format("TrustManagement", JSON.stringify(params));
                    ...
                    if (response[0].result) {
                        if (callback)
                            ...
                            switch (response[0].result) {
                                case 'EXIST':
                                    $('.form-control').eq(0).addClass('red-border');
                                    $('.form-control').eq(1).addClass('red-border');
                                    $('.form-control').eq(2).addClass('red-border');
                                    GSDialog.HintWindow("当前权益人信息已存在，请确认填写的权益人信息");
                                    break;
                                case 'INSERT': GSDialog.HintWindow("新增权益人信息成功"); break;
                                case 'UPDATE': GSDialog.HintWindow("更新权益人信息成功"); break;
                            }
                        }
                            //console.log(11);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response, function (i, v) {
                        v.ItemValue = common.numFormt(v.ItemValue)
                    })
                    console.log(response)
