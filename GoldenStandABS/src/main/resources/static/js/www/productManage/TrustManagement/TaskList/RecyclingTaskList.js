...
define(function (require) {
    ...
    function RenderOption(data,resc) {

        var optiontemp = "<option value='{0}'>{1}</option>"
        var selectobj = $('#dtNextPeriod')
        var resign = [];
        $.each(resc, function (c, v) {
            resign.push(v.CyclDate)
        })

        //
        $.each(data, function (i, n) {
            if ($.inArray(n.enddate, resign) >= 0) {
                return true;
            }
            var html = ''
            html = optiontemp.format((n.enddate + '_' + n.Num), n.enddate);
    ...
    function loadTrustPeriodTaskList(callback) {
        var svcUrl = GlobalVariable.DataProcessServiceUrl + 'CommonGetExecute?';
        var sessionNamePrefix = '{0}|_{1}|_'.format(r_trustId, r_taskCode);
        var objArgs = {
            SPName: 'usp_ReadCyclesManage',
            SQLParams: [{ Name: 'TrustId', Value: r_trustId, DBType: 'int' }]
        };
        var executeParams = encodeURIComponent(JSON.stringify(objArgs));
        $.ajax({
            cache: false,
    ...
    function renderListItem(source) {
        ...
        $.each(source, function (i, v) {
            html += gridRowTemplate.format(v.CyclNum, v.CyclDate, v.StartDate, operatorTemplate.format(v.CyclDate, v.CyclesManageId));
        });
    ...
    openSessionTask = function openSessionTask(rDate) {
    ...
    deleteSessionTask = function deleteSessionTask(sId) {
        GSDialog.HintWindowTF("确定删除？", function () {
            var svcUrl = GlobalVariable.DataProcessServiceUrl + 'CommonGetExecute?';
            var objArgs = {
                SPName: 'usp_DeleteCyclesManage',//'usp_DeleteSession',
    ...
    function createNewSession() {
        ...
        var sContext = {
            ...
            sessionVariables: "<SessionVariables>"
                ...
                + "<SessionVariable><Name>ReportingDate</Name><Value>" + rDatelist[0] + "</Value><DataType>String</DataType><IsConstant>0</IsConstant><IsKey>1</IsKey><KeyIndex>0</KeyIndex></SessionVariable>"
        ...
        wProxy.createSessionByTaskCode(sContext, function (response) {
            ...
            prepareTrustAssociatedDatesSessionContext(response, rDatelist[0], rDatelist[1]);
        });
    }
    function prepareTrustAssociatedDatesSessionContext(targetSessionId, reportingDate,CycNum) {
        ...
        var sContext = {
            ...
            sessionVariables: "<SessionVariables>"
                ...
                + "<SessionVariable><Name>CycNum</Name><Value>" + CycNum + "</Value><DataType>String</DataType><IsConstant>0</IsConstant><IsKey>0</IsKey><KeyIndex>0</KeyIndex></SessionVariable>"
                + "</SessionVariables>",
            taskCode: 'NewTrustAssociatedDatesPreparation'
        };
        var wProxy = webProxy;
        wProxy.createSessionByTaskCode(sContext, function (response) {
    ...
    $(function () {
        ...
        loadTrustPeriodTaskList(function (res) {
            var sourceData;
            ...
            //////////////
            loadCyclDate(sourceData, function (res, resc) {
                var sourceData;
                if (typeof resc === 'string') { sourceData = JSON.parse(resc); }
                else { sourceData = resc; }

                RenderOption(sourceData,res);
