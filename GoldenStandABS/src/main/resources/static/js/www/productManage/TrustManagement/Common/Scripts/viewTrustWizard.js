...
define(function (require) {
    ...
    var GSdialog =require('anyDialog');
    ...
    TRUST = {
        ...
        //任务单校验功能刷新现金流模型
        refreshCashflowModelTask: function (TrustCode) {
            var self = this;
            var trustId;
            ...
            $.ajax({
                ...
                success: function (response) {
                    trustId = jQuery.parseJSON(response)[0].TrustId;
                    var sContent = "{'SPName':'usp_GetTrustInfoFromWizard','Params':{" +
                            "'TrustId':'" + trustId +
                            "'}}";
                    var serviceUrl = config.tmsSessionServiceBase + "GetItemsPlus?applicationDomain=TrustManagement&contextInfo=" + sContent;
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var schedulePurpose = common.getQueryString('schedulePurpose') ? common.getQueryString('schedulePurpose') : 1;
                            ...
                            if (common.getQueryString('scheduleDate')) {
                                var scheduleDateId = common.getQueryString('scheduleDate');
                                sVariableBuilder.AddVariableItem('ScheduleDateId', scheduleDateId, 'String', 0, 0, 0);
                            }
                            sVariableBuilder.AddVariableItem('TrustCode', TrustCode, 'String', 0, 0, 0);
                            sVariableBuilder.AddVariableItem('IsRevolving', isRevolving, 'String', 0, 0, 0);
                            sVariableBuilder.AddVariableItem('ScenarioId', ScenarioId, 'String', 0, 0, 0);

                            Exchangeinfo = Exchangeinfo ? Exchangeinfo : "InterBankMarket";
        ...
        //产品管理刷新现金流模型
        refreshCashflowModelManage: function () {
        ...
        //产品设计刷新现金流模型
        refreshCashflowModelDesign: function () {
            var trustCode = TRUST.getItemValueByCode('TrustItem', 'TrustCode');
            var isRevolving = TRUST.getItemValueByCode('TrustItem', 'IsTopUpAvailable');
        ...
        if (productPermissionState == 2) {
            ...
            var tIndicator = new taskIndicator({
                ...
                callback: function () {
                    ...
                    var go;
                    var fileName = '收益分配数据校验.xlsx';
                    var executeParamEX = {
                        SPName: 'Verification.usp_GetVerificationStatusForCashflow', SQLParams: [
