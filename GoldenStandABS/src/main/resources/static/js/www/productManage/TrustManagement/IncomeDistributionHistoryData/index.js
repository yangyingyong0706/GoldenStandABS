...
define(function (require) {
    ...
    Vue.filter('FormatMoney', function(p) {
        if (p !== undefined && p !== null)
            if (parseFloat(p) === 0) {
                ...
                p = parseFloat(p).toFixed(2);// 避免因为浮点数运算出现的小数位数过多的问题
    ...
    var vm = new Vue({
        ...
        data: {
            ...
            sumInfoForBondCodeData: [],
            trustBondCode: [],
            selectPeriodModel: 0,
            isRightStartEndDate: true,
            isSelected: false,
            selectedStart: 0, // 预留，选中时间轴区间
            selectedEnd: '',
            selectedPeriod: '',
            sumStartDate: '',
            sumEndDate: '',
            ...
            tips: [
                ...
                '上期费用预提取但未支付的金额',
                '当期费用预提取但未支付的金额',
                '在收款期间内,总体回流的利息金额(包含资产回收款、资产赎回款、清仓回购款、其他收入等)',
                '当期总支付的税金和费用(包括中登费)',
                '在收款期间内,总体回流的本金金额(包含资产回收款、资产赎回款、清仓回购款、其他收入等)',
                '当期总支付的证券利息和证券本金',
                '即为“期初资产池未偿本金总额”',
                '即为“期末资产池未还本金总额”'
            ]
        ...
        methods: {
            setSelectedEnd: function(e) {
                this.Periods.forEach((element, index) => {
                    if(element.EndDate === e.target.value)
                        this.selectedEnd = index;
            });
},
    setSelectedStart: function(e) {
        this.Periods.forEach((element, index) => {
            if(element.EndDate === e.target.value)
...
selectPeriod: function(e) {
    this.GetAllData(this.Periods[e.target.value].EndDate);
    this.ShowPeriodDetail(e.target.value, this.Periods[e.target.value])
    // this.Periods.forEach((element, index) => {
    //     if(element.EndDate === e.target.value) {
    //         this.GetAllData(e.target.value);
    //         this.ShowPeriodDetail(index, element);
    //     }     
    // });
},
// 封装一个AJax请求方法
...
changeIsSum: function (isSum) {
    self = this;
    self.isSum = isSum;
},
getNowSumBondCode: function(bondCode, event) {
    var self = this;
    if(event !== undefined) {
        var $li = $(event.currentTarget);
        $li.addClass('active').siblings().removeClass('active');
    } else {
        var $li = $(".TrustBond li").eq(self.trustBondCode.length);
        $li.addClass('active').siblings().removeClass('active');
    }
    self.sumInfoForBondCodeData = [];
    this.sumData.sumInfoForBondCode.forEach(element => {
        if(element.TrustBondCode === bondCode)
            self.sumInfoForBondCodeData.push(element);
});
console.log(this.sumInfoForBondCodeData);
},
getSum: function() {
    self = this;
    var params = [
        { Name: 'TrustId', value: self.Periods[0].TrustId, DBType: 'int' },
        { Name: 'StartDate', value: self.getDataStringForSql(self.sumStartDate), DBType: 'string' },
        { Name: 'EndDate', value: self.getDataStringForSql(self.sumEndDate), DBType: 'string' },
    ];
    var executeParam = {
        SPName: 'TrustManagement.usp_IsRightStartEndDate', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        if(res[0].result === 1) {
            self.isRightStartEndDate = true;
            $("#DC_TimeLine li").slice(self.Periods.length, self.Periods.length * 2 - 2).removeClass('selected');
            $("#DC_TimeLine li").slice(self.selectedStart + self.Periods.length, self.selectedEnd + self.Periods.length + 1).addClass('selected');
            self.isSelected = true;
        } else if(self.sumEndDate === '' || self.sumStartDate === '') {
            return;
        } else {
            self.isRightStartEndDate = false;
        }
    });
    if(this.isRightStartEndDate === false)
        return; 
    var executeParam = {
        SPName: 'TrustManagement.usp_GetSumFactTrustTransactionAccountInfo', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        self.sumData.accountInfo = res;
    });
    var executeParam = {
        SPName: 'TrustManagement.usp_GetSumFactTrustTransactionCashInflow', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        self.sumData.cashInflows = res;
    });
    var executeParam = {
        SPName: 'TrustManagement.usp_GetSumFactTrustTransactionFee', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        self.sumData.trustTransactionFee = res;
    });
    var executeParam = {
        SPName: 'TrustManagement.usp_GetSumFactBondPaymentBalance', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        self.sumData.bondBalanceParam = res;
    });
    var executeParam = {
        SPName: 'TrustManagement.usp_GetSumInfoForBondCode', 
        SQLParams: params
    };
    self.getSourceData(executeParam, function (res) {
        self.trustBondCode = [];
        self.sumData.sumInfoForBondCode = res;
        res.forEach((element, index) => {
            if(index !== 0) {
                if(res[index-1].TrustBondCode !== element.TrustBondCode)
                    self.trustBondCode.push(element.TrustBondCode);
            } else {
                self.trustBondCode.push(element.TrustBondCode);
            }
        });
    });
    this.getNowSumBondCode(self.sumData.sumInfoForBondCode[0].TrustBondCode);
},
//将日期格式化为yyyy-xx-mm
...
//将日期格式化为yyyyxxmm
getDataStringForSql: function (date) {
...
//时间轴点击获取index值和更改样式
ShowPeriodDetail: function (index, period) {
    var self = this;
    $("#DC_TimeLine li").eq(index).addClass('selected').siblings().removeClass('selected');
    self.nowIndex = index;
    webStorage.setItem('nowPeriod', index);
    self.selectPeriodModel = index;
},
//现金交易流明细重组数组包括helps
...
getNowSumBondCode: function(bondCode, event) {
    var $li = $(event.currentTarget);
    $li.addClass('active').siblings().removeClass('active');
    let sumInfoForBondCodeData = [];
    this.sumData.sumInfoForBondCode.forEach(element => {
        if(element.TrustBondCode === bondCode)
            sumInfoForBondCodeData.push(element);
});
this.sumInfoForBondCodeData = sumInfoForBondCodeData;
},
getSum: function() {
    self = this;
    var params = [
    ...
    this.getNowSumBondCode(sumData.sumInfoForBondCode[0].TrustBondCode);
},
//将日期格式化为yyyy-xx-mm
GetDateString: function (date) {
    var Date = common.getStringDate(date);
