define(function (require) {
    ...
    function RefreshSelectAppRoles() {
        RoleArray.length = 0;
        RoleOperate.getPathsRoles(PathId, function (data) {
    ...
    function saveItems() {
        ...
        if (RolesIds.length == 0) { GSDialog.HintWindow("请选择角色！"); }
            for (var i = 0; i < RolesIds.length; i++) {
                xml += '<Item><PathId>' + PathId + '</PathId><RoleId>' + RolesIds[i] + '</RoleId></Item>';
            }
            xml += '</Items>';
            RoleOperate.savePathRoles(xml, function (r) {
                if (r == '1') {
                    //$("#roles").val("");
    ...
    this.getOperate = function (PathId, roleId) {
        var html = "";
        html = '<button class="btn btn-link" onclick="self.deleteRole(\'' + PathId + '\',\'' + roleId + '\')"><i class="fa fa-trash-o" title="删除"></i></button>';
        return html;
    }
                    ...
                    if (operation == "read") {
                        ...
                        if (dataSource.filter() != null) {
                            ...
                            var parameter = {
                                ...
                                tableName: RoleOperate.schema + '.[ViewPathsRoles]',
                                page: options.page,
    ...
    this.deleteRole = function (PathId, roleId) {
        GSDialog.HintWindowTF("确定删除此角色？", function () {
            RoleOperate.deletePathsRoleById(PathId, roleId, function (r) {
                if (r == '0') {
                    //刷新数据
